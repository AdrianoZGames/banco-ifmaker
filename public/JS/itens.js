    ...
    fetch('api/itens')
        ...
        .then((data) => {
            ...
            for (let item of data) {
                ...
                tdDisponivel.textContent =
                    item.disponivel <= 0 ? 'Indisponível' : 'disponível'
                let tdDataDeAquisicao = document.createElement('td')
                tdDataDeAquisicao.textContent = formatarData(
                    item['data-de-aquisicao']
                )
...
function criarItens(event) {
    ...
    const dataConvertida = new Date(dataDeAquisicao).toISOString().split('T')[0]

    let item = {
        nome: nome,
        disponivel: disponivel,
        data_de_aquisicao: dataConvertida,
    }
    console.log(item)
    fetch(url, {
        method: metodo,
